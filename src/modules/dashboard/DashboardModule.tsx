...
export default function DashboardModule() {
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-6xl mx-auto pb-24 px-4 md:px-8 text-left antialiased">
      ...
      <div className="relative overflow-hidden rounded-[32px] border border-border/50 bg-surface/30 backdrop-blur-md shadow-sm grid grid-cols-12 items-center p-8 md:p-10 gap-6 min-h-[160px]">
        <div className="col-span-12 md:col-span-8 flex flex-col gap-2 text-left z-10 w-full">
          <h2 className="text-2xl md:text-3xl font-black leading-tight tracking-tight text-text-primary">
            Focus on what matters<span className="text-rose-500">.</span>
          </h2>
          <p className="text-xs text-text-secondary leading-relaxed font-medium mt-1 max-w-xl">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 w-full">
        <div className="bg-surface border border-border/50 rounded-2xl p-4 flex items-center gap-3.5 shadow-sm">
          <div className="w-9 h-9 rounded-xl bg-emerald-500/10 flex items-center justify-center text-emerald-500 border border-emerald-500/10 shrink-0">
            <IconClockPlay className="w-4 h-4" />
          </div>
          <div>
            <p className="text-xs text-text-muted font-bold uppercase tracking-wider">Focus Time</p>
            <p className="text-lg font-black text-text-primary font-mono tracking-tight mt-0.5">{focusTimeLabel}</p>
          </div>
        </div>

        <div className="bg-surface border border-border/50 rounded-2xl p-4 flex items-center gap-3.5 shadow-sm">
          <div className="w-9 h-9 rounded-xl bg-rose-500/10 flex items-center justify-center text-rose-500 border border-rose-500/10 shrink-0">
            <IconCheck className="w-4 h-4 stroke-[2.5]" />
          </div>
          <div>
            <p className="text-xs text-text-muted font-bold uppercase tracking-wider">Tasks Ratio</p>
            <p className="text-lg font-black text-text-primary font-mono tracking-tight mt-0.5">{completedTasksCount}/{totalTasks}</p>
          </div>
        </div>

        <div className="bg-surface border border-border/50 rounded-2xl p-4 flex items-center gap-3.5 shadow-sm">
          <div className="w-9 h-9 rounded-xl bg-orange-500/10 flex items-center justify-center text-orange-500 border border-orange-500/10 shrink-0">
            <IconFlame className="w-4 h-4" />
          </div>
          <div>
            <p className="text-xs text-text-muted font-bold uppercase tracking-wider">Consistency</p>
        ...
        <div className="bg-surface border border-border/50 rounded-2xl p-4 flex items-center gap-3.5 shadow-sm">
          ...
          <div>
            <p className="text-xs text-text-muted font-bold uppercase tracking-wider">Focus Streak</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 w-full items-start">
        ...
        <div className="bg-surface border border-border/50 rounded-3xl p-6 flex flex-col gap-6 shadow-sm relative overflow-hidden h-full min-h-[380px]">
          <div className="flex items-center justify-between w-full">
            <div className="flex items-center gap-2.5">
              <div className="w-7 h-7 rounded-lg bg-emerald-500/10 flex items-center justify-center text-emerald-500 border border-emerald-500/10">
          ...
          <div className="flex gap-2 w-full mt-auto">
            ...
            <button onClick={stopGlobalPomodoro} disabled={pomodoroTimerState === 'idle'} title="Reset Timer"
              className="w-10 h-10 rounded-xl bg-surface-alt hover:bg-surface-hover border border-border text-text-secondary flex items-center justify-center transition-all disabled:opacity-30 cursor-pointer shrink-0 active:scale-95">
        ...
        <div className="bg-surface border border-border/50 rounded-3xl p-6 flex flex-col gap-4 shadow-sm h-full min-h-[380px]">
          <div className="flex items-center justify-between w-full shrink-0">
            <div className="flex items-center gap-2.5">
              <div className="w-7 h-7 rounded-lg bg-rose-500/10 flex items-center justify-center text-rose-500 border border-rose-500/10">
          ...
          <form onSubmit={handleAddTask} className="flex gap-2 w-full shrink-0">
            ...
            <button type="submit" disabled={!newTaskTitle.trim()}
              className="w-9 h-9 rounded-xl bg-rose-500 hover:bg-rose-600 text-white flex items-center justify-center transition-all disabled:opacity-30 cursor-pointer shrink-0 active:scale-95">
        ...
        <div className="flex flex-col gap-6 h-full w-full">
          ...
          {/* Habits Mini widget */}
          <div className="bg-surface border border-border/50 rounded-3xl p-5 flex flex-col gap-4 shadow-sm w-full">
            <div className="flex items-center justify-between w-full">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-black uppercase tracking-wider text-text-primary">Daily Habits</span>
              </div>
              ...
              {dueHabits.slice(0, 2).map(habit => {
                ...
                return (
                  <button key={habit.id} onClick={() => toggleHabitCompletion(habit.id, todayStr)}
                    className="flex items-center gap-3 px-3 py-2 rounded-xl bg-surface-alt/45 hover:bg-surface-alt border border-border/20 text-left transition-all w-full group cursor-pointer">
                    <div className={`w-4 h-4 rounded-full border-[1.5px] flex items-center justify-center shrink-0 transition-all ${
          ...
          <div className="bg-surface border border-border/50 rounded-3xl p-5 flex flex-col gap-4 shadow-sm w-full flex-grow">
            <div className="flex items-center justify-between w-full">
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 rounded-lg bg-purple-500/10 flex items-center justify-center text-purple-500 border border-purple-500/10">
                  <IconSitemap className="w-3.5 h-3.5" />
                </div>
                <span className="text-xs font-black uppercase tracking-wider text-text-primary">Mindmaps</span>
              </div>
            ...
            <div className="flex flex-col gap-2 overflow-y-auto max-h-[140px] pr-1">
              {mindmaps.slice(0, 2).map(m => (
                <button key={m.id} onClick={() => handleOpenMindmap(m.id)}
                  ...
                  <div className="w-7 h-7 rounded-lg bg-purple-500/5 text-purple-500 flex items-center justify-center shrink-0 border border-purple-500/10">
                    <IconSitemap className="w-3.5 h-3.5" />
                  </div>
                  <div className="min-w-0 flex-grow">
                    <p className="text-xs font-bold text-text-primary truncate leading-none">{m.title}</p>
