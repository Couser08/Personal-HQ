...
export const EmptyState = ({ icon, title, description, action }: EmptyStateProps) => (
  ...
  >
    ...
    <div
      aria-hidden="true"
      className="mb-4 text-text-tertiary flex items-center justify-center transition-colors duration-200"
    >
      {icon ?? <IconInbox className="w-10 h-10 stroke-[1.25]" />}
    </div>

    {/* Title: Sharp, condensed system typography */}
    <h3 className="text-[14.5px] font-semibold text-text-primary tracking-tight">
      {title}
    </h3>

    {/* Description: Balanced leading with maximum line control */}
    {description && (
      <p className="mt-1 text-[12.5px] text-text-secondary font-normal leading-relaxed max-w-[280px] mx-auto">
        {description}
      </p>
    )}
